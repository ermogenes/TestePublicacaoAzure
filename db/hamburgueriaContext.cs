...
namespace TestePublicacaoAzure.db
{
    public partial class hamburgueriaContext : DbContext
    {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            if (!optionsBuilder.IsConfigured)
            {
                ...
                string server = !dbhost.Contains(":")? dbhost : dbhost.Split(':')[0].ToString();
                string port = !dbhost.Contains(":") ? "": "port=" + dbhost.Split(':')[1].ToString();
                ...
                string connectionString2 = $@"server={server};user={dbusername};password={dbpassword};database={dbname};{port};";
